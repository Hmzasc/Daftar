...
let screenStack = ["screen-welcome"]; // مسار التنقل الحقيقي، لدعم زر "رجوع" بدقة

// ============================================================
// التنقل بين الشاشات
// ============================================================
// ============================================================
// التنقل بين الشاشات (مع مسار حقيقي يدعم "رجوع" بدقة)
// ============================================================
window.showScreen = function (id, options = {}) {
  ...
  if (options.reset) {
    screenStack = [id];
  } else if (screenStack[screenStack.length - 1] !== id) {
    screenStack.push(id);
  }
};
...
window.goBack = function () {
  if (screenStack.length > 1) {
    screenStack.pop(); // أزل الشاشة الحالية
...
window.logout = async function () {
  ...
  selectedLinkId = null;
  lastAddedLinkId = null;
  selectedRole = null;
  showScreen("screen-welcome", { reset: true });
};
...
// يحوّل أي صيغة لرقم الهاتف إلى صيغة موحّدة دائماً: أرقام فقط،
// مع رمز الدولة الموريتاني 222 دائماً في البداية (حتى لو لم يكتبه المستخدم)
// هذا التوحيد ضروري كي يتطابق نفس الرقم دائماً بغض النظر عمّن كتبه
// أو كيف كتبه (بمسافات، برمز +، بدون رمز الدولة...)
...
// صيغة عرض مقروءة لرقم موحّد، للتأكيد البصري في الواجهة فقط
...
window.registerNew = async function () {
  ...
  try {
    ...
    console.log("[دفتر] تم إنشاء حساب جديد. uid:", currentUser.uid, "| الرقم المخزَّن:", currentUserPhone);
    await afterLogin();
  } catch (err) {
...
window.loginExisting = async function () {
  ...
  try {
    ...
    currentUser = result.user;
    // نجلب الرقم من Firestore مباشرة (وليس من متغيّر محلي) لضمان تطابقه
    // تماماً مع ما تعتمده قواعد الأمان (myPhone) عند أي عملية لاحقة
    const userDoc = await getDoc(doc(db, "users", currentUser.uid));
    currentUserPhone = userDoc.exists() ? userDoc.data().phone : phone;
    console.log("[دفتر] تسجيل دخول ناجح. uid:", currentUser.uid, "| الرقم المخزَّن في Firestore:", currentUserPhone);
    await afterLogin();
  } catch (err) {
...
async function afterLogin() {
  if (selectedRole === "seller") {
    ...
    if (snap.empty) {
      showScreen("screen-shop-setup", { reset: true });
    } else {
      ...
      document.getElementById("seller-phone-display").textContent = displayPhone(currentUserPhone);
      showScreen("screen-seller-dashboard", { reset: true });
      listenSellerDashboard();
    }
  } else {
    document.getElementById("customer-phone-display").textContent = displayPhone(currentUserPhone);
    const linkDisplay = document.getElementById("link-screen-phone-display");
    if (linkDisplay) linkDisplay.textContent = displayPhone(currentUserPhone);
    showScreen("screen-customer-dashboard", { reset: true });
...
onAuthStateChanged(auth, async (user) => {
  if (user && !currentUser && selectedRole) {
    ...
    console.log("[دفتر] استعادة جلسة سابقة تلقائياً. uid:", user.uid, "| الرقم:", currentUserPhone);
    afterLogin();
  }
});
...
window.linkShop = async function () {
  ...
  console.log("[دفتر] محاولة ربط بالكود:", code, "| uid الحالي:", currentUser?.uid, "| رقمي المسجَّل:", currentUserPhone);
  ...
  try {
    ...
    console.log("[دفتر] رقم الزبون المسجَّل على هذا الكود من طرف البائع:", data.customerPhone);

    if (data.customerPhone !== currentUserPhone) {
      errorEl.textContent = `هذا الكود مخصَّص لرقم مختلف عن رقمك الحالي (${displayPhone(currentUserPhone)})`;
      console.warn("[دفتر] عدم تطابق: customerPhone=", data.customerPhone, "!== currentUserPhone=", currentUserPhone);
      return;
    }
